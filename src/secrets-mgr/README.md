# IBMCloud MCP Server - Secrets Manager

This project provides a Model Context Protocol (MCP) server with tools for managing secrets using Secrets Manager and can be configured
alongside other MCP servers eg. git or filesystem MCP Servers to allow llm's to assist with secrets scanning.

## ⚙️ Configurations

The following example MCP configurations can be found in the `configs` folder to help get started with secrets manager use cases:

- [mcp.sec-mgr.json](https://github.com/IBM-Cloud/ibmcloud-mcp-server/blob/main/src/serverless/configs/mcp.code_engine_apps.json) - Tools commonly used when working with Secrets Manager

These configurations also include the following essential tools:
